...
(begin-for-syntax
  ...
  (define (begin-reorder? stx-list)
    ...
    ;; range: (or/c exact-integer? (cons/c (and/c (</c 0) exact-integer?) exact-positive-integer?))
    (define (make-range v1 v2)
      (define (merge v p)
        (cond ((zero? v) p)
              ((> v 0) (cons (car p) (max v (cdr p))))
              (else (cons (min (car p) v) (cdr p)))))
      (cond ((and (pair? v1) (pair? v2))
             (cons (min (car v1) (car v2))
                   (max (cdr v1) (cdr v2))))
            ((pair? v1) (merge v2 v1))
            ((pair? v2) (merge v1 v2))
            ((< (* v1 v2) 0)
             (cons (if (< v1 0) v1 v2) (if (< v1 0) v2 v1)))
            ((> (abs v1) (abs v2))
             v1)
            (else v2)))
                                  ...
                                  (syntax-parse st
                                    ...
                                    ((loop s ...)
                                     ...
                                     (and (zero? rest)
                                          ...
                                          old))
                                    (op:shift
                                     (define r (+ old (get-offset shift #'op)))
                                     (make-range old r))
                                    (_ #f)))))
                       0
                       (apply append blocks))))))
